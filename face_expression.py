...
mdls_dir = cur_dir + "/models/"
predictor_path = os.path.join(
    mdls_dir, 'shape_predictor_68_face_landmarks.dat')
...
while(1):
    ...
    for index, face in enumerate(dets):
        ...
        features = np.zeros(img2.shape[0:-1], dtype=np.uint8)
        for index_, pt in enumerate(shape.parts()):
            pt_pos = (pt.x, pt.y)
        ...
        # faceROI = features[face.top():face.bottom(),
        # face.left():face.right()]
        faceROI = cv2.resize(faceROI, (500, 500),
