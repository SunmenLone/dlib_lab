...
def eye_aspect_ratio(eye):
    dist_p1p4 = np.sqrt((eye[0][0] - eye[3][0])**2 +
                        (eye[0][1] - eye[3][1])**2)
    dist_p2p6 = np.sqrt((eye[1][0] - eye[5][0])**2 +
                        (eye[1][1] - eye[5][1])**2)
    dist_p3p5 = np.sqrt((eye[2][0] - eye[4][0])**2 +
                        (eye[2][1] - eye[4][1])**2)
    return (dist_p2p6 + dist_p3p5) / (2 * dist_p1p4)


cur_dir = os.getcwd()  # 获取当前路径
model_path = os.path.join(cur_dir, 'models')  # 模型文件夹路径
shape_detector_path = os.path.join(
    model_path, 'shape_predictor_68_face_landmarks.dat')  # 人脸特征点检测模型路径
...
while (1):
    ...
    for rect in rects:  # 遍历每一个人脸
        ...
        cv2.putText(img, "Blinks:{0}".format(blink_counter), (10, 30),
