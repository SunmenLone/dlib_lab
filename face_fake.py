...
class FaceChanger(object):
    def __init__(self, which_predictor='68'):
        ...
        predictor_68_points_path = self.current_path\
            + '/models/shape_predictor_68_face_landmarks.dat'
        predictor_5_points_path = self.current_path\
        ...
        print(
            'You can choose two of theses pictures,\
                and change the face between them.'
        )
        ...
        self.OVERLAY_POINTS = [
            ...
            self.RIGHT_EYE_POINTS + self.LEFT_EYE_POINTS,
            self.NOSE_POINTS + self.MOUTH_POINTS,
        ]
    ...
    def run(self, showProcedure=False, saveResult=True):
        ...
        if showProcedure is True:
            print(
                'Showing the procedure.Press any key to continue your process.'
            )
        ...
        M = self.transformation_from_points(
            self.landmarks1[self.ALIGN_POINTS],
            self.landmarks2[self.ALIGN_POINTS]
        )
        ...
        combined_mask = np.max([
            self.get_face_mask(self.image1, self.landmarks1),
            warped_mask],
            axis=0
        )
        if showProcedure is True:
        ...
        warped_corrected_img2 = self.correct_colours(self.image1, warped_img2,
                                                     self.landmarks1)
        warped_corrected_img2_temp = np.zeros(
            warped_corrected_img2.shape, dtype=warped_corrected_img2.dtype)
        cv2.normalize(warped_corrected_img2, warped_corrected_img2_temp, 0, 1,
                      cv2.NORM_MINMAX)
        if showProcedure is True:
        ...
        output = self.image1 * (
            1.0 - combined_mask) + warped_corrected_img2 * combined_mask
    ...
    def get_landmark(self, image):
        ...
        if len(face_rect) > 1:
            ...
            print('left {0}; top {1}; right {2}; bottom {3}'.format(
                face_rect[0].left(), face_rect[0].top(), face_rect[0].right(),
                face_rect[0].bottom()))
            # box = face_rect[0]
            ...
            return np.matrix(
                [[p.x, p.y]
                 for p in self.predictor(image, face_rect[0]).parts()])
    ...
    def get_face_mask(self, img, landmarks):
        ...
        img = (cv2.GaussianBlur(
            img, (self.FEATHER_AMOUNT, self.FEATHER_AMOUNT), 0) > 0) * 1.0
        img = cv2.GaussianBlur(img, (self.FEATHER_AMOUNT, self.FEATHER_AMOUNT),
                               0)
